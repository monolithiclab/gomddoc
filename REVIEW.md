...
- **Status:** 15th pass — two HIGH findings remain open (§10.2's URL/output-path divergence, §10.3's
  ignored `get_table_of_contents` path and redirect-finder argument); roughly 80 MEDIUM/LOW findings
  remain open across §9.8, §10, §11 and §12 (bugs found by the 2026-09-25 documentation audit), counting
  each item of a LOW cluster separately (listed under each subsection). Last re-verified against the code on
  2026-09-25. **The prioritized remediation plan lives in `docs/roadmap.md`'s Implementation Strategy
  section** — this file tracks findings, that file tracks what to do about them and in what order.
...
- **Branch:** main
- **Go Version:** 1.26 (toolchain 1.26.8)
...
- ~~**15 govulncheck-reachable vulnerabilities; no vuln gate in CI**~~ ✅ FIXED. Four modules bumped;
  `make lint-vulncheck` (part of `make lint`, which every CI run executes) now gates this. See
...
- **MEDIUM (open): `/sitemap-index.xml` is generated by build but 404s in serve.** Build writes it;
  serve never registers a route for it (`NewHTTPServer` says so in a comment and keeps `robots.txt`
  consistent by never advertising it).
- **MEDIUM (open): per-language tag pages gated on a domain in build only.** Serve gates per-language
  tag routes on `MetaIndex != nil && LocaleBundle != nil` (no domain condition); `BuildCmd.Run` calls
  `emitTagPages` for a translation only inside the `cfg.Site.Meta.Domain != ""` block that writes
  that language's sitemap and feed. A no-domain multi-language site serves tags fine but ships a
  build missing the translated ones, with every tag chip on a translated page pointing at a 404.
...
- **LOW (open): search results link to raw `.md` paths** — `internal/search/index.go` stores the raw
  path and `cmd/gomddoc/assets/shared/search.mjs` uses it directly as `href`; same class as the
  (fixed) §10.3 tag-list finding, but search has no resolver dependency, so the mapping belongs in
...
- ~~**The bundled `default` theme forks `head-meta` and loses hreflang**~~ ✅ FIXED — `head.html.tmpl`
  ...
  - **MEDIUM (open): tag pages render with nil `Languages` and nil `Features`.** `RenderTagPage` and
    `RenderTagsIndex` (`internal/template/renderer.go`) build their context with
    `WithI18n(lang, tFunc, nil)` and without going through `config.MergeFeatures`, so a site with
...
- **HIGH (open, verified): two declared-and-documented parameters are silently ignored.**
  `handleGetTOC` (`internal/mcp/tools.go`, the `get_table_of_contents` tool) never reads
  `GetTOCInput.Path`, despite it being in the generated JSON Schema and documented in
  `docs/guide/04-mcp.md` — a client asking for one subtree gets the whole site with no error.
  `cmd/gomddoc/pipeline.go`'s `redirectFinderAdapter` ignores its path argument and always does a
  global DFS from the root, so `/guide/` with no index redirects to the site's first page, not the
  first page under `/guide/`.
- ~~**Six advertised environment variables are inert**~~ ✅ FIXED — `NewFromServeArgs` now calls
  ...
  - **LOW (open): preview's `--dir-index` answers to two different env var names**
    (`GOMDDOC_DIR_INDEX` on the Kong flag vs `GOMDDOC_SITE_DIR_INDEX` on the struct tag) — same class
    as `GOMDDOC_DOMAIN` vs `GOMDDOC_SITE_META_DOMAIN`. Pick one naming scheme. Tracked again as §11.1.
- ~~**MEDIUM: `findRelatedDocs` bypasses the documented single source of truth for tag
  normalization.**~~ ✅ FIXED (`3754610`, 2026-08-07). `findRelatedDocs` still passes the raw tag, but
  `Index.PagesByTag`, `ByTag` and `CountByTag` now key the lookup through `normalizeTag` (lowercase
  plus trim), the same function `BuildIndex` keys the map with, so `"  Deployment  "` finds its pages.
- **MEDIUM (open): index pages appear in their own "related docs" — serve only.** Serve passes
  `r.URL.Path` to `Enrich` (`internal/server/handler.go`), so the self-exclusion key is `/guide/`;
  the index stores `/guide/README.md`. Build is correct (uses the file path), so serve and build
  render different see-also blocks for index pages.
...
- **MEDIUM (open): git submodules classified as directories → `build` aborts.** `treeDirEntries`
  (`internal/provider/gitfs.go`) sets `isDir: !entry.Mode.IsFile()`, which is also true for
  `filemode.Submodule`; a gitlink causes `fs.WalkDir` to descend, `resolveTreeNode` then fails to
  load the gitlink's hash as a blob, and the whole build aborts. Should be
  `entry.Mode == filemode.Dir`, which `resolveTreeNode` and `statTreeNode` already use.
- **MEDIUM (open): `.well-known` silently dropped from git-backed sites.** The git provider's
  hidden-file filter in `treeDirEntries` is a blanket `strings.HasPrefix(entry.Name, ".")`, missing
  the RFC 8615 `.well-known` exception `provider.IsHiddenPath` honours. Direct `ReadFile` still
  works, so the divergence is silent. (Same finding as §10.13's third bullet.)
- ~~**MEDIUM: search results nondeterministically ordered on ties.**~~ ✅ FIXED (`4edc17b`,
  2026-08-04) — `compareScored` (used by `rankTopN`) breaks score ties on `docIdx`. **Still open
  (related):** `feed.go` sorts by mtime with no secondary key, so entries whose mtimes tie (universal
  after a fresh clone) are ordered by whatever order `AllPages` happens to return rather than by a
  stated rule.
- **MEDIUM (open): snippet body truncated on a byte boundary → invalid UTF-8** in non-ASCII
  documents (`maxSnippetBody = 8192` bytes, `snippetBody[:maxSnippetBody]` in `BuildIndex`, no
  rune-boundary re-alignment).
- **MEDIUM (open): canonical URLs for non-root index pages lack the trailing slash the build
  serves** — `normalizePagePath` (`internal/seo/url.go`) only adds the slash at the root; build
  writes `docs/index.html` (real URL `/docs/`) but the canonical says `/docs`.
- ~~**Config loading is silently permissive**~~ ✅ FIXED — `LoadFromFile` now decodes with
  ...
  - **LOW cluster, still open (same class, lower severity):** locale override typos silently ignored
    (`Bundle.loadFrom` in `internal/locale/bundle.go`); at startup `SiteConfig.Validate` checks a
    feature key's shape, not membership (`color_chip: true` singular passes and does nothing) —
    `ValidateFeatureKeys` was folded into it in `21f487c`, and `gomddoc doctor` reports the unknown
    toggle as `theme.unknown-feature` since `e217faf`, but `serve`/`build` stay silent; yaml.v3 error
    messages leak Go type names (`LoadFromFile` wraps them verbatim, and doctor's `config.wrong-type`
    finding repeats them); `config.ConfigFile` now holds the display path, but
    `filepath.Join(dir, ConfigDirName, ConfigFileName)` is still spelled out at 3 call sites
    (`LoadFromFile`, `readConfigNode`, `InitCmd.Run`), plus a `path.Join` form in
    `cmd/gomddoc/capabilities.go`.
- **MEDIUM (open): divergent frontmatter parsers.** `metadata.FrontmatterBlock` (behind
  `extractFrontmatter`) tolerates leading whitespace and an indented closing `---`;
  `text.StripFrontmatter` requires both at column 0 — so for those inputs,
...
request-scoped `context.Background()` in the breadcrumb `isDir` closure built in
`cmd/gomddoc/pipeline.go` (uncancellable git clone). Git-read follow-ups, all LOW: directory-index
reads (`GitProvider.handleDirectoryLocked`) bypass the `maxFileSize`/LFS guards `readFileLocked`
applies; `gitTreeFS.Open` has no size cap; depth-2+ paths defeat go-git's subtree memo; three
slightly-inconsistent "is this a directory" checks remain.
...
- **LOW: guide blocks transcribe a shipped data structure with nothing pinning them.** Pinned so
  far: the translation-key list (`TestBuiltinTranslationKeysAreDocumented`), the `GOMDDOC_*` env var
  names and config keys (`TestGuide_MentionsOnlyRealEnvVars`, `TestGuide_ConfigurationPageCoversEverySetting`,
  `06b4ee9`), and the doctor finding codes (`TestDoctorGuide_MatchesCatalogue`). Still unpinned: the
  template functions, the `<gmd-*>` element table, and the MCP tools/resources/prompts list
  (duplicated in *two* guide pages). Generalizing to one table-driven test over
  `{docPath, anchor, extractor}` rows is the fix, not one-off tests.
...
- **LOW: build re-derives the language sub-FS the pipeline already built** — `fs.Sub(contentRoot,
  lang)` in `walkAndBuildLang` (`build.go`) duplicates work `setupLanguagePipelines` already did.
- **LOW: three copies of the "fail one path" `fs.FS` test fake** (`unreadableDirFS` in
  `internal/resolve`, `erroringFS` in `internal/metadata`, `subFailFS` in `cmd/gomddoc`) — candidate
  for an `internal/testutil/failfs` extraction.
...
- **Helper duplication** (CLAUDE.md "one canonical test helper per pattern"): `internal/config` got
  its `testhelpers_test.go` (`writeSiteConfig`, `b039e26`), but three tests still hand-roll the
  `.gomddoc/` directory (`config_test.go`, `env_test.go`, `inspect_test.go`); `internal/template`
  re-inlines a default-layout MapFS 38 times; two tests in `internal/server/server_test.go`
  hand-roll a renderer registry missing `NewMarkdownPassthroughRenderer()` instead of using
  `internal/testutil/registry`, silently exercising a different registry than every other test in
  that package.
- **Missing benchmarks on hot paths:** `metadata.BuildIndex`, `resolve` lookup,
  `template/breadcrumb`, `locale.Bundle.T`. (`internal/provider` gained git read benchmarks in
  `git_reads_test.go`, `4d4701b`; the filesystem provider still has none.)
...
- **NEW: unmatched `/api/*` routes answer HTML, not JSON** — `GET /api/nope` falls through to the
  content scope and returns its themed `text/html` 404, so a JSON client parsing an unexpected-path
  error gets a decode failure instead of `{"error": ...}`. Fix: register
...
- **NEW: `TemplateCache` models a boolean as a two-implementation interface** — with `Clear()`
  ...
  not a defect.
- **NEW: `AllPages` has no `iter.Seq` sibling** — 6 read-only consumers still deep-copy the whole
  corpus (`server/sitemap.go`, `server/feed.go`, `server/redirect.go`, `search.buildMetaLookup`,
  `mcp/resources.go`, `guide.New` in `internal/guide/guide.go`); MCP's `handleListPages` also
  collect-all-then-truncates against CLAUDE.md's bounded-results rule.
- **NEW: `mcp/tools.go handleFindRelated` (the `find_related` tool) reimplements
  `enricher.findRelatedDocs`** without the bound or sort, so the MCP answer and the rendered "related
  docs" block can disagree.
...
- **Extensive documentation verified accurate** — config tables, precedence rules, HTTP semantics
  ...
  resources/3 prompts at the time (the self-documentation and doctor work since brought `gomddoc
  mcp` to 9 tools/7 resources/4 prompts), the asset-overlay precedence, search UX. The release pipeline is internally
...
- **LOW: `?lang=` is an exact, case-sensitive lookup** (`ResolveAPILanguage`,
  `internal/server/language.go`) — `?lang=fr-fr` silently falls back to the
...
- `mcp.ServerDeps.ExcludePatterns` (set from `cfg.Site.Exclude` by `siteMCPServer` in
  `cmd/gomddoc/pipeline.go` and by `MCPCmd.Run` in `mcp.go`) ignores `lp.Default`'s effective list,
  so MCP's restricted-path gate doesn't know about excluded language directories.
- `internal/server/server.go`'s `ContentExclusion` middleware takes `cfg.Site.Exclude` for the
  default content scope, the same gap — reachable if a language pipeline fails to build and falls
  through to the default scope.
- `treeDirEntries` in `internal/provider/gitfs.go` duplicates `IsHiddenPath` without its
  `.well-known` exception (same finding as §10.3's `.well-known` bullet).

Also open, LOW: the three exclusion-pattern branches of `IsExcludedPath` re-derive their structure
per call instead of compiling once at pipeline construction.

~~`DetectMIME`'s process-global registry works only because every caller happens to import
`negotiate`~~ ✅ FIXED (`2bfa95c`, 2026-08-07) — the `mime.AddExtensionType` calls moved into
`internal/negotiate/mime.go`, the package that owns `DetectMIME`, so every caller links them.

### 10.14 NEW — every translated page advertises the default-language feed

**MEDIUM, open:** `<link rel="alternate">` for the feed is built via `canonicalURL "/feed.xml"`
(`partials/head-shared.html.tmpl`), which — unlike
...
`gomddoc help --search` already strips them, but locally in `writeSearch` (`cmd/gomddoc/help.go`); strip them in one
place for all three consumers.
...
level-1 heading, returning half a section. Fixed in `1c10b21` with a fence-aware `scanHeadings` shared with
`HeadingIDs`; both now live in `internal/text/section.go`.

### 11.4 FALSE POSITIVE — `--domain` bypasses the domain validation

**Verified 2026-09-25, not a bug.** `--domain` is applied after `NewFromServeArgs` (`setupServer` in `pipeline.go`)
or `NewFromDir` (`BuildCmd.Run`) has validated the config, and both call sites then run `cfg.Site.Validate()` on the
new value, which applies the same `meta.domain` checks as the file. `gomddoc build --domain https://x/y` and
`gomddoc serve --domain https://x/y` both exit with `domain flag: domain should not include protocol: https://x/y`.
Folding the flag into `NewFromServeArgs` would remove the second `Validate` call but changes no behavior.

### 11.5 LOW — duplicate heading anchors

**Open.** goldmark suffixes duplicate heading IDs (`setup`, `setup-1`); `slugifyHeading` (`internal/text/section.go`)
does not, so `read_section`/`gomddoc_guide` can only reach the first of two same-named sections, and `HeadingIDs`
lists the duplicate twice.

## 12. Found During the Documentation Audit (2026-09-25)

Every guide page, spec and plan was checked against the code on 2026-09-25. These are the code bugs that check
turned up. Each was either reproduced against a running `serve`/`build` or read directly from the code, as marked.
Where the guide describes current behavior that is one of these bugs, it says so in a warning. Update those
warnings when the bug is fixed.

### 12.1 MEDIUM — Git sources

- **`--git-storage-dir` cannot restart (reproduced).** The second run against the same directory fails with
  `clone …: repository already exists`. `cloneLocked`/`ensureCloned` (`internal/provider/git.go`) always call
  `CloneContext` and never open an existing repository, so disk storage gives no cache reuse and breaks restarts.
- **Only branch refs work (reproduced).** `#v1.6.0` or `#<sha>` fails with `couldn't find remote ref
  "refs/heads/…"`: the clone uses `plumbing.NewBranchReferenceName` with no tag or commit fallback, although the
  comments and `resolveCommitLocked` claim all three. The guide and website document branch-only pinning until fixed.
- **`config.yml` in a Git source is never read (read from code).** `LoadFromFile` joins the Git URL as an OS path
  (`internal/config/config.go`), so a committed `.gomddoc/config.yml` is ignored while `.gomddoc/assets/` in the same
  repo is used. `exclude`, `strip_extensions` and `theme.vars` have no env var, so a Git-sourced site cannot set them.
- **LOW: the default title of a Git site is the URL (observed in logs).** `titleFromDir(URL)` yields titles such as
  `Hello-World.git#7Fd1…`. Derive it from the repository name, or the subdirectory when one is given.
- **LOW: `doctor` passes a site whose clone failed.** A clone failure is `target.read-error`, a warning, so
  `gomddoc doctor` exits 0 on a broken Git source unless `--strict` is set.

### 12.2 MEDIUM — translated pages

- **The language switcher links the default language to a 404 (reproduced, serve and build).**
  `partials/lang-switcher.html.tmpl` renders every inactive language as `/{code}{path}`; the default language has
  no prefix, so `/fr-FR/guide` links English to `/en-US/guide`. `hreflang.html.tmpl` already handles `.Default`.
- **Translated pages lose the site's `.gomddoc/` overlay in serve (reproduced).** `setupLanguagePipelines`
  (`cmd/gomddoc/pipeline.go`) builds each language's asset overlay from `fs.Sub(root, lang)`, so an installed theme
  under `.gomddoc/assets/themes/`, `.gomddoc/partials/` and `.gomddoc/assets/shared/` overrides are invisible to it.
  `/fr-FR/…` renders with `default` and logs `Theme template not found, falling back to default`. Build renders
  translated content through the default pipeline's renderer, so content pages are themed but `fr-FR/tags/*` is not.
- **Search results in a language tree link to default-language pages (reproduced).** `/api/search?lang=fr-FR`
  returns paths without the `/fr-FR` prefix, and `search.mjs` never sends `lang`, so the index is chosen from the
  browser's `Accept-Language` rather than the page's language.
- **Canonical URL and `og:url` on translated pages lack the language prefix in serve too (reproduced).** §10.2
  records it for build only. Same root cause as §10.14: `canonicalURL` does not apply the language prefix.
- **A non-en-US default language without a locale file shows raw keys (reproduced).** With `language: de-DE` and
  no `de-DE` bundle, the UI renders `aria_search`, `toc_title` and so on: the fallback chain ends at the configured
  language and never reaches the built-in en-US strings.
- **LOW: hreflang links are root-relative, and emitted for every language whether or not the page is translated
  (read from template).** Google requires absolute URLs, and an alternate pointing at an untranslated page is a 404.

### 12.3 MEDIUM/LOW — other

- **MEDIUM: the edit link can point at the wrong file (read from code).** The handler passes the request URL
  (`r.URL.Path`, `internal/server/handler.go`) as `.Page.Path`, which `editURL` appends to `edit_url`. With
  `strip_extensions`, or on a directory index page, the link loses `.md` or `README.md`.
- **LOW: `get_table_of_contents` has no `path` handling (see §10.3), and `search_docs` has no query length cap.**
  Every other MCP string argument is capped at 1024 bytes.
- **LOW: tag-page bodies ignore site partials.** `partialTemplate` (used for `tags-list`/`tags-index`) does not
  layer `.gomddoc/partials/`, unlike layout parsing.
- **LOW: build writes `guide.md` and `guide/README.md` to the same `guide/index.html` with no warning.**
  `prettyOutputPath` needs the collision check the resolver already reports for served URLs.
- **LOW: the HSTS branch in `SecurityHeaders` is dead code.** It checks `r.TLS != nil`, and gomddoc never
  terminates TLS. Remove it, or document that HSTS belongs on the TLS proxy (the guide now does the latter).
- **LOW: JSON-LD `SearchAction` targets `/api/search`,** a JSON endpoint, not a results page (`internal/seo/jsonld.go`).
- **LOW: `tags_empty` is unreachable.** An unknown tag 404s before the tags-list partial runs, and the tags index
  with zero tags renders an empty list instead of the string.
- **LOW: `gomddoc info` lists `--dir-index` for `site.dir_index` without saying only `preview` has that flag.**
- **By design (confirmed 2026-09-25):** `/metrics` on a separate `--admin-port` is served without Basic Auth even
  when `--basic-auth-file` is set (pprof keeps it). The admin port must never be exposed publicly; the guide says so
  in warnings (07, 08, 09, 12-advanced/03).
- **Question, not yet a bug:** site-wide `meta.robots: noindex` does not empty the sitemap or feed; only frontmatter
  `robots` does.

### 12.4 gomddoc-themes (not gomddoc code)

The seven downloadable themes lag the default theme: none sets the `data-search-*` attributes on `<html>` (so the
`tag:` search tip and other search strings are missing), none calls `.T` or `lang-switcher` (UI strings are
hardcoded English, `<html lang>` comes from `.Site.Language`, not `.Lang`), none ships `error.html.tmpl`, and their
`.heading-anchor` CSS is dead since anchors became `<gmd-heading-anchor>`.
